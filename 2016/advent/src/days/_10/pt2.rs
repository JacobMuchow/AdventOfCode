...
pub fn run() {
    ...
    let chips0 = &output_bins.get(&0).unwrap().chips;
    let chips1 = &output_bins.get(&1).unwrap().chips;
    let chips2 = &output_bins.get(&2).unwrap().chips;

    // The description seems to suggest each output will only have 1 chip so I am going to berify that.
    assert_eq!(chips0.len(), 1);
    assert_eq!(chips1.len(), 1);
    assert_eq!(chips2.len(), 1);

    let product = chips0[0] * chips1[0] * chips2[0];
    println!("Product of output bins 0, 1, and 2: {}", product);
}