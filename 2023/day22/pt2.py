...
def chainReaction(bricks, num):
    # print(f'Chain: {num}')
    # if num in memo:
    #     print(f'memo {num}: {memo[num]}')
    #     return memo[num]
    
    count = 0
    for i in bricks[num]["up"]:
        bricks[i]["down"].remove(num)

        if len(bricks[i]["down"]) == 0:
            count += 1 + chainReaction(bricks, i)
...
for i in range(0, len(bricks)):
    count = chainReaction(deepcopy(bricks), i)
    print(f'Brick {i}: {count}')
    total += count
