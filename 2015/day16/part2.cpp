...
void Day16Part2::run(std::string inputFile) {
    ...
    for (auto& sue : sues) {
        ...
        for (auto& entry : sue->props) {
            ...
            if (entry.first == "cats" || entry.first == "trees") {
            ...
            else if (entry.first == "pomeranians" || entry.first == "goldfish") {
                if (entry.second >= analysis[entry.first]) {
                    validSue = false;
                    break;
                }
            ...
            else if (entry.second != analysis[entry.first]) {
                validSue = false;
                break;
            }
        }
