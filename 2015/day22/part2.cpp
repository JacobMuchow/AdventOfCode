...
int Day22Part2::spellCost(Spell spell) {
    switch (spell) {
...
void Day22Part2::printState(GameState state) {
    if (!DEBUG) return;
...
GameCondition Day22Part2::checkGameCondition(GameState state) {
    if (state.player.hp <= 0) return Defeat;
    if (state.boss.hp <= 0) return Victory;
    return Ongoing;
}

GameState Day22Part2::processEffects(GameState state) {
    // Shield
...
GameState Day22Part2::playerAction(GameState state, Spell spell) {
    if(DEBUG) cout << "-- Player turn --" << endl;
    printState(state);

    // Add in the 1hp deduction per player turn
...
GameState Day22Part2::bossAction(GameState state) {
...
void Day22Part2::testScenario(GameState gameState, std::vector<Spell> playerActions) {
...
int Day22Part2::leastManaRecursive(GameState curState, int leastMana) {
    ...
    for (auto& spell : spellList) {
        ...
        // I made an error the first time around. You can still cast these on the turn they end,
        // in which case it is valid for "turnsLeft" to be 1.
        if (spell == Shield && curState.shieldEffect.turnsLeft > 1) continue;
        if (spell == Poison && curState.poisonEffect.turnsLeft > 1) continue;
        if (spell == Recharge && curState.rechargeEffect.turnsLeft > 1) continue;
...
void Day22Part2::run(std::string inputFile) {
    // Test 1
