...
void Day07Part2::run(std::string inputFile) {
    ...
    while (!solveQueue.empty()) {
        ...
        if (c->type() == "BASIC") {
            ...
            if (valueMap.find(conn->in) != valueMap.end()) {
                valueMap[conn->out] = getValue(conn->in);
                continue;
            }
            solveQueue.push(c);
        }
            ...
            if (valueMap.find(gate->in) != valueMap.end()) {
                valueMap[gate->out] = ~getValue(gate->in);
                continue;
            }
            solveQueue.push(c);
        }
            ...
            if (valueMap.find(gate->in1) != valueMap.end() && valueMap.find(gate->in2) != valueMap.end()) {
                valueMap[gate->out] = getValue(gate->in1) & getValue(gate->in2);
                continue;
            }
            solveQueue.push(c);
        }
            ...
            if (valueMap.find(gate->in) != valueMap.end()) {
                valueMap[gate->out] = getValue(gate->in) >> gate->val;
                continue;
            }
            solveQueue.push(c);
        }
...
bool Day07Part2::addToValueMapIfNumerical(std::string name) {
...
ushort Day07Part2::getValue(std::string name) {
    // Override value for "b" to previous solution for "a".
    if (name == "b") return 3176;
