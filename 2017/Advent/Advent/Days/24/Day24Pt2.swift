...
class Day24Pt2 {
    static func run() {
        ...
        let (maxStrength, maxLen) = tryParts(bridge: &bridge, partPool: &partPool, usedParts: &usedParts)
        
        print("Max length: \(maxLen), Max strength: \(maxStrength)")
    }
    
    private static func tryParts(bridge: inout Bridge, partPool: inout [Int: [Part]], usedParts: inout Set<Int>) -> (Int, Int) {
        var maxStrength = bridge.strength
        var maxLen = bridge.parts.count
        ...
        for part in possibleParts {
            ...
            let (strength, len) = tryParts(bridge: &bridge, partPool: &partPool, usedParts: &usedParts)
            if (len == maxLen) {
